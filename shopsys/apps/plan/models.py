...
class Plan(models.Model):
    ...
    toctiy=models.CharField("目的城市",max_length=30,blank=False)
    ...
    update_at= models.DateTimeField("修改时间", auto_now_add=True)
...
class Scheme(models.Model):
    name = models.CharField("方案名称", max_length=50)
    traffictype = models.IntegerField("交通工具")
    hotletype = models.IntegerField("住宿")
    start_at = models.DateTimeField("出发时间")
    arrive_at = models.DateTimeField("到达时间")
    total_time=models.IntegerField("总时间")
    total_fee=models.DecimalField("总费用",max_digits=9,decimal_places=2,blank=True,default=0.00)
    description = models.TextField("描述")
    is_active = models.IntegerField("方案状态", default=True)
    create_at = models.DateTimeField("创建时间", auto_now_add=True)
    update_at = models.DateTimeField("修改时间", auto_now_add=True)
...
class Plane_plan(models.Model):
    ...
    start_at = models.DateTimeField("起飞时间", auto_now_add=True)
    arrive_at = models.DateTimeField("落地时间", auto_now_add=True)
...
class Plane_scheme(models.Model):
    name = models.CharField("航班名称", max_length=50)
    companyname = models.CharField("航空公司", max_length=50)
    planetype = models.CharField("执行机型", max_length=50)
    fromcity = models.CharField("起飞站", max_length=30, unique=True)
    tocity = models.CharField("落地站", max_length=30, unique=True)
    start_at = models.DateTimeField("起飞时间", auto_now_add=True)
    arrive_at = models.DateTimeField("落地时间", auto_now_add=True)
    fromstation = models.CharField("起飞机场航站", max_length=50)
    tostation = models.CharField("落地机场航站", max_length=50)
    ...
    create_at = models.DateTimeField("创建时间", auto_now_add=True)
    update_at = models.DateTimeField("修改时间", auto_now_add=True)
    planid=models.ForeignKey("Scheme")

    class Meta:
        db_table = 'plane_scheme'
        verbose_name = '飞机方案'
        verbose_name_plural = verbose_name
...
class Train_plan(models.Model):
    ...
    fromstation = models.CharField("出发站", max_length=30, unique=True)
    tostation = models.CharField("到达站", max_length=30, unique=True)
    start_at = models.DateTimeField("发车时间", auto_now_add=True)
    arrive_at = models.DateTimeField("到站时间", auto_now_add=True)
...
class Train_scheme(models.Model):
    name = models.CharField("车次", max_length=50)
    traintype = models.CharField("火车类型", max_length=20)
    fromstation = models.CharField("出发站", max_length=30, unique=True)
    tostation = models.CharField("到达站", max_length=30, unique=True)
...
class Hotel_plan(models.Model):
    ...
    address = models.CharField("地址", max_length=30, unique=True)
    coordinate = models.CharField("地址坐标", max_length=60)
    classtype = models.CharField("房间类型", max_length=20)
    price=models.DecimalField("价格",max_digits=9,decimal_places=2,blank=True,default=0.00)
    start_at = models.DateField("入住日期", auto_now_add=True)
    end_at = models.DateField("离店日期", auto_now_add=True)
...
class Hotel_scheme(models.Model):
    name = models.CharField("宾馆名称", max_length=50)
    hoteltype = models.CharField("类型", max_length=20)
    address = models.CharField("地址", max_length=30, unique=True)
    coordinate = models.CharField("地址坐标", max_length=60)
    ...
    start_at = models.DateField("入住日期", auto_now_add=True)
    end_at = models.DateField("离店日期", auto_now_add=True)
    total_time = models.FloatField("住宿天数")
    description = models.TextField("描述")
    is_active = models.IntegerField("订购状态", default=True)
    create_at = models.DateTimeField("创建时间", auto_now_add=True)
    update_at = models.DateTimeField("修改时间", auto_now_add=True)
